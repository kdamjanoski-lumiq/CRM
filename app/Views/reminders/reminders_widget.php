<script type="text/javascript">
    ...
    setTimeout(() => getMissedRemindersCount(), 30000); //check missed reminders after 30 seconds

    showReminderCountIconOnTopbar = function(count) {
        var badge = "";
        if (count && count > 0) {
        ...
        IDBHelper.setValue("reminder_count", count);
    };
        ...
        appAjaxRequest({
            ...
            success: function(result) {
                if (result.success && result.total_reminders && result.total_reminders * 1) {
                    showReminderCountIconOnTopbar(result.total_reminders);
                }
            }
        });
    };
    ...
    $(document).ready(async function() {
        ...
        var reminderCount = await IDBHelper.getValue("reminder_count");
        if (reminderCount && reminderCount * 1) {
            showReminderCountIconOnTopbar(reminderCount);
        }
        ...
        function showReminder(reminder) {
            ...
            if (AppHelper.https === "1") {
                ...
                NotificationHelper.showBrowserNotification(data);
            } else {
                //app notification
                ...
                appAlert.warning(reminder.title + " " + actionButtonsDom, {
                    duration: 300000
                }); //show reminder for 5 minutes
            }
        }
        ...
        $("body").on("click", "[data-act='snooze-reminder']", function() {
            appLoader.show();
            appAjaxRequest({
                url: "<?php echo get_uri('events/snooze_reminder') ?>",
                type: 'POST',
                dataType: 'json',
                data: {
        ...
        $("body").on("click", "[data-act='dismiss-reminder']", function() {
            ...
            appAjaxRequest({
                url: "<?php echo get_uri('events/save_reminder_status') ?>/" + $(this).attr('data-id'),
                type: 'POST',
                dataType: 'json',
                data: {
                    value: "shown"
