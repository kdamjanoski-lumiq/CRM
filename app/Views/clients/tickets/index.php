...
<?php if ($page_type === "full") { ?>
    ...
    <div class="card">
        ...
        <div class="table-responsive <?php echo $page_type === 'full' || $view_type != 'overview' ? '' : 'no-display-length client-view-tickets-card' ?>">
            <table id="ticket-table" class="display <?php echo $page_type === 'full' || $view_type != 'overview' ? '' : 'no-thead b-t b-b-only no-hover hide-dtr-control' ?>" width="100%">
            </table>
        </div>
    </div>
    <?php if ($page_type === "full") { ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var pageType = "<?php echo isset($page_type) && $page_type === "full" ? "full" : ""; ?>",
            viewType = "<?php echo isset($view_type) && $view_type === "overview" ? "overview" : ""; ?>",
            url = "<?php echo get_uri("tickets/ticket_list_data_of_client") . "/" . $client_id . "/0/1/widget" ?>",
            filterDropdown = [],
            mobileMirror = true,
            stateSave = false,
            responsive = true;
        if (pageType == "full" || viewType != "overview") {
            console.log("here");
